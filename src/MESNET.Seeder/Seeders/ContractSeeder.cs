...
public static class ContractSeeder
{
    public static async Task SeedAsync(MesnetApiClient api, SeedContext ctx)
    {
        ...
        // Contract 1: Active — tam imzalı
        await SeedContract1(api, ctx, institutionId, now);

        // Contract 2: AwaitingSignature — kısmen imzalı
        await SeedContract2(api, ctx, institutionId, now);

        // Contract 3: Draft — yeni oluşturulmuş
        await SeedContract3(api, ctx, institutionId, now);

        // Contract 4: Completed — tamamlanmış
        await SeedContract4(api, ctx, institutionId, now);
    }

    private static async Task SeedContract1(MesnetApiClient api, SeedContext ctx, Guid institutionId, DateTime now)
    {
    ...
    private static async Task SeedContract2(MesnetApiClient api, SeedContext ctx, Guid institutionId, DateTime now)
    {
    ...
    private static async Task SeedContract3(MesnetApiClient api, SeedContext ctx, Guid institutionId, DateTime now)
    {
        if (!ctx.Has("Student4") || !ctx.Has("Business2")) return;

        var data = await api.PostAsync("/api/contracts", new
        {
            studentId = ctx.Get("Student4"),
            businessId = ctx.Get("Business2"),
            institutionId,
            teacherId = ctx.Has("Teacher2") ? ctx.Get("Teacher2") : (Guid?)null,
    ...
    private static async Task SeedContract4(MesnetApiClient api, SeedContext ctx, Guid institutionId, DateTime now)
    {
        if (!ctx.Has("Student3") || !ctx.Has("Business1")) return;

        var data = await api.PostAsync("/api/contracts", new
        {
            studentId = ctx.Get("Student3"),
            businessId = ctx.Get("Business1"),
            institutionId,
            teacherId = ctx.Has("Teacher1") ? ctx.Get("Teacher1") : (Guid?)null,
