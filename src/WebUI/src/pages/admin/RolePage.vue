<template>
  <q-page padding>
    <div class="text-h5 text-weight-bold">Rol Yetkileri</div>
        ...
        <q-card flat bordered class="full-height">
          <q-card-section>
            <div class="row items-center q-mb-xs no-wrap">
              <q-icon :name="roleMeta(role.roleName).icon" color="primary" size="24px" class="q-mr-sm" />
              <div class="text-subtitle1 text-weight-bold">{{ roleMeta(role.roleName).name }}</div>
            </div>
            <div class="text-caption text-grey-7 q-mb-md">{{ roleMeta(role.roleName).desc }}</div>

            <div class="text-caption text-weight-medium text-grey-8 q-mb-xs">Yapabilecekleri</div>
            <q-chip
