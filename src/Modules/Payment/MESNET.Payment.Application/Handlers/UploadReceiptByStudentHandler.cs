...
public static class UploadReceiptByStudentHandler
{
    ...
    public static async Task<(Guid, ReceiptUploadedByStudent)> Handle(
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            loggerFactory.CreateLogger("UploadReceiptByStudent")
                .LogError("MinIO upload başarısız: {Error}", uploadResult.Error.Description);
            throw new DomainException(uploadResult.Error);
        }

        loggerFactory.CreateLogger("UploadReceiptByStudent")
            .LogInformation("Dekont öğrenci tarafından yüklendi: SalaryPeriodId={SalaryPeriodId}, ReceiptId={ReceiptId}",
                command.SalaryPeriodId, receiptId);

        return (receiptId, new ReceiptUploadedByStudent(
            command.SalaryPeriodId, receiptId, objectPath, DateTime.UtcNow));
    }
}
