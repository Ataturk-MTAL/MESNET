...
/// <summary>
/// <c>AttendanceReportConsumer</c>'ın yerel kuyruğu <b>sıralı</b> olmalı (#262).
///
...
/// </summary>
public sealed class AttendanceReportConsumerQueueOrderingTests
{
    [Fact]
    public void Kuyruk_yapilandirmasi_taniml()
    {
        typeof(IConfigureLocalQueue).IsAssignableFrom(typeof(AttendanceReportConsumer))
            .ShouldBeTrue(
    ...
    {
        var handlerMetotlari = typeof(AttendanceReportConsumer)
    ...
    private static string SourcePath() => Path.Combine(RepoRoot(), "src/Modules/Reporting/MESNET.Reporting.Application/Consumers/AttendanceReportConsumer.cs");
