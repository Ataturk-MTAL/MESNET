...
/// <summary>
/// Token'dan gelen <c>business_id</c> <b>hiçbir koşulda</b> kabul edilmez (#229).
///
...
/// </summary>
public sealed class BusinessClaimAuthorityTests
{
    ...
    /// </summary>
    [Fact]
    public async Task Kayit_bos_olsa_bile_tokendaki_isletme_atilir()
    {
        var transformation = Create(provider: null);

        var result = await transformation.TransformAsync(Principal(tokenBusinessId: Spoofed));

        result.FindFirst("business_id").ShouldBeNull();
    }

    [Fact]
    public async Task Kayit_doluysa_tokendaki_isletme_yerine_kayit_gecerlidir()
    {
        var transformation = Create(new StubBusinessProvider(Recorded));

        var result = await transformation.TransformAsync(Principal(tokenBusinessId: Spoofed));

        result.FindAll("business_id").Select(c => c.Value).ShouldBe([Recorded.ToString()]);
    }

    [Fact]
    public async Task Tokende_claim_yokken_kayittaki_isletme_eklenir()
    {
        var transformation = Create(new StubBusinessProvider(Recorded));

        var result = await transformation.TransformAsync(Principal(tokenBusinessId: null));

        result.FindFirst("business_id")?.Value.ShouldBe(Recorded.ToString());
    }
    ...
    /// </summary>
    [Fact]
    public async Task Ikinci_identity_uzerindeki_sahte_isletme_de_atilir()
    {
        var transformation = Create(provider: null);

        var principal = Principal(tokenBusinessId: null);
        principal.AddIdentity(new ClaimsIdentity(
            [new Claim("business_id", Spoofed.ToString())], "second"));
    ...
    /// <summary>
    /// <b>Kapı kapalı kalmalı:</b> hiçbir kod Keycloak'a <c>business_id</c> özniteliği
    /// yazmamalıdır.
    ///
    ...
    /// </summary>
    [Fact]
    public void Hicbir_kod_keycloaka_business_id_ozniteligi_yazmaz()
    {
        ...
        foreach (var file in Directory.EnumerateFiles(sourceRoot, "*.cs", SearchOption.AllDirectories))
        {
            ...
            for (var i = 0; i < lines.Length; i++)
            {
                ...
                var key = lines[i].IndexOf("[\"business_id\"]", StringComparison.Ordinal);
                if (key < 0) continue;
        ...
        violations.ShouldBeEmpty(
            "Keycloak'a business_id özniteliği yazılıyor. Otorite UserAccount.BusinessId'dir; "
    ...
    private static PermissionClaimsTransformation Create(StubBusinessProvider? provider)
    {
    ...
    private static ClaimsPrincipal Principal(Guid? tokenBusinessId)
    {
        var claims = new List<Claim> { new("sub", Sub) };
        if (tokenBusinessId is { } id)
            claims.Add(new Claim("business_id", id.ToString()));

        return new ClaimsPrincipal(new ClaimsIdentity(claims, "test"));
    }

    private sealed class StubBusinessProvider(Guid businessId) : IUserPermissionProvider
    {
